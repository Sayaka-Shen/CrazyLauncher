...
{
	m_centralLayout->addWidget(m_projectView);
	m_centralLayout->addWidget(m_descView);
...
Project* CrazyLauncher::GetSelectedProjectWidget()
{
	...
	if (!item)
	{
		std::cout << "There are not items selected" << std::endl;
		return nullptr;
	}
	...
	if (!itemWidget)
	{
		std::cout << "There are not item widget" << std::endl;
		return nullptr;
	}
